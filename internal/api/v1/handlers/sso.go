...
func (h *handler) enrichOIDCConfig(ctx context.Context, oidcConfig *dockyardsv1.OIDCConfig) error {
	if oidcConfig.ProviderConfig != nil {
		return nil
	}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, *oidcConfig.ProviderDiscoveryURL, nil)
	if err != nil {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("discovery endpoint returned %s", res.Status)
	}

	var discovered dockyardsv1.OIDCProviderConfig
	err = json.NewDecoder(res.Body).Decode(&discovered);
	if err != nil {
		return fmt.Errorf("decode discovery document: %w", err)
	}

	oidcConfig.ProviderConfig = &discovered
	return nil
}
...
func (h *handler) Callback(w http.ResponseWriter, r *http.Request) error {
	...
	if len(userList.Items) > 1 {
		return apierrors.NewInternalError(fmt.Errorf("expected exactly one user, but found many", "len", len(userList.Items)))
	}
