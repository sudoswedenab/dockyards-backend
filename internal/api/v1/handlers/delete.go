...
func DeleteClusterResource(h *handler, resource string, f DeleteClusterResourceFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = f(ctx, &cluster, resourceName)
		if client.IgnoreNotFound(err) != nil {
...
type DeleteOrganizationResourceFunc func(context.Context, *dockyardsv1.Organization, string) error

func DeleteOrganizationResource(h *handler, resource string, f DeleteOrganizationResourceFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		...
		err = f(ctx, &organization, resourceName)
		if client.IgnoreNotFound(err) != nil {
			logger.Error("error deleting resource", "err", err)
			w.WriteHeader(http.StatusInternalServerError)
